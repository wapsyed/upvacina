...
## Microrregião: Cobertura vacinal somente

```{r}
pca_microrregiao = IEPS_Completo_2010_2022_selecionados_SP %>% 
  filter(nivel == "Região",
         ano == "2020") %>%
  select(no_regiao, cob_vac_bcg:cob_vac_penta) %>% 
  column_to_rownames("no_regiao")


pca_microrregiao_matrix = pca_microrregiao %>% 
  as.matrix()

pca_microrregiao_ann = IEPS_Completo_2010_2022_selecionados_SP %>% 
  filter(nivel == "Região",
         ano == "2020") %>% 
  select(no_regiao, pop) %>% 
    column_to_rownames("no_regiao")
  
# Delete columns with near 0 variance
nearZeroVarCols <- nearZeroVar(pca_microrregiao_matrix, saveMetrics = TRUE)
pca_microrregiao_matrix_pca <- pca_microrregiao_matrix[, !nearZeroVarCols$nzv]
pca_res <- prcomp(pca_microrregiao_matrix_pca, scale. = T)

# Correlation plot ----
corr_matrix = cor(pca_microrregiao_matrix_pca %>% scale()) 
...
circle_contrib %>% 
  ggsave(file = here("Figuras", "PCA_Microrregioes_2020_Loadings.png"), 
         width = 5, 
         height = 5)
...
pca_microrregiao$cluster <- as.factor(cluster_model$cluster)
pca_microrregiao = pca_microrregiao %>% 
  rownames_to_column("id")


# Plot ------
pca_plot_knn_cluster = autoplot(pca_res, 
        data = pca_microrregiao, 
        colour = 'cluster')  +
...
pca_plot_knn_cluster %>% ggsave(file = here("Figuras", "PCA_Microrregioes_2020.png"), width = 10, height = 6)
...
pca_plot_knn_cluster_gradient %>% ggsave(file = here("Figuras", "PCA_Microrregioes_2020_Polio.png"), width = 10, height = 6)
```

```{r fig.width = 10, fig.height = 6}
pca_plot_knn_cluster_pop = autoplot(pca_res, 
        data = pca_microrregiao %>% 
          bind_cols(pca_microrregiao_ann) %>% 
          rownames_to_column("id"))  +
...
pca_plot_knn_cluster_pop

pca_plot_knn_cluster_pop %>% ggsave(file = here("Figuras", "PCA_Microrregioes_2020_Pop.png"), width = 10, height = 6)
```
...
corrplot_selec = corrplot(corr_matrix_selec, is.corr=TRUE,
         tl.col = "black",
         method = "color",
          col=colorRampPalette(c("blue","white","red"))(9)) +
    theme(axis.text.x = element_text(angle = 90, size = 10),
        axis.text.y = element_text(size = 10))

corrplot = ggcorrplot(corr_matrix_selec, hc.order = TRUE) +
  theme(axis.text.x = element_text(angle = 90, size = 10),
        axis.text.y = element_text(size = 10))
corrplot

corrplot %>% 
  ggsave(file = here("Figuras", "PCA_Microrregioes_2020_Selecionados_Corrplot.png"), width = 10, height = 10)
...
scree_plot %>% 
  ggsave(file = here("Figuras", "PCA_Microrregioes_2020_Selecionados_Screeplot.png"), 
         width = 5,
         height = 3)
```
...
circle_contrib %>% 
  ggsave(file = here("Figuras", "PCA_Microrregioes_2020_Selecionados_Loadings.png"), 
         width = 5, 
         height = 5)
...
pca_plot_knn_cluster_marginal %>% ggsave(file = here("Figuras", "PCA_Microrregioes_2020_Selecionados_Clusters.png"), width = 10, height = 6)
...
pca_plot_knn_cluster_gradient %>% ggsave(file = here("Figuras", "PCA_Microrregioes_2020_Selecionados_Polio.png"), width = 10, height = 6)
```
...
pca_plot_knn_cluster_biplot = fviz_pca_biplot(pca_res, 
                ...
                fill.ind = pca_microrregiao$cluster,
                col.ind = "black",
                pointshape = 21,
...
pca_plot_knn_cluster_biplot %>% ggsave(file = here("Figuras", "PCA_Microrregioes_2020_Selecionados_Biplot.png"), width = 10, height = 6)
```
...
pop_regiao = IEPS_Completo_2010_2022_selecionados_SP %>%
  filter(nivel == "Região") %>% 
    mutate(porte_municipio = case_when(pop <= 20000 ~ "Pequeno Porte I",
                                     ...
                                     pop <= 900000 ~ "Grande Porte",
                                     TRUE ~ "Metrópole")) %>% 
  select(no_regiao, porte_municipio)


dados_vacinas = IEPS_Completo_2010_2022_selecionados_SP %>% 
  filter(nivel == "Região") %>%
  select(no_regiao, ano, cob_ab:cob_vac_penta, 
         tx_mort_aj_cens:pct_desp_recp_saude_mun, 
         desp_tot_saude_pc_mun_def,
         desp_recp_saude_pc_mun_def, 
         ideb_5ano:pop, 
         pct_pop_fem:pct_pop_masc) %>% 
  inner_join(pop_regiao, by = "no_regiao") %>% 
  distinct()
```
...
cor_vac_ideb = dados_vacinas %>% 
  ...
  select(c("no_regiao", "ideb_5ano", "ideb_9ano", "cob_vac_bcg", "ano", "porte_municipio", "pop")) %>% 
  ggplot(.) +
...
cor_vac_ideb %>% 
  ggsave(file = here("Figuras", "Microrregioes_2010_2020_BCG_vs_IDEB.png"), 
         width = 20,
         height = 10)
...
corrplot %>% 
  ggsave(file = here("Figuras", "PCA_Municípios_2020_Selecionados_Corrplot.png"), width = 10, height = 10)
...
cor_vac_txmortes = dados_vacinas %>% 
  ...
  theme(legend.position = "right",
        ...
        legend.text = element_text(size = 15),
        strip.text.x = element_text(size = 15, face = "plain"))  +
  labs(title = "Cobertura vacinal da BCG e Taxa de mortes evitaveis (CENSO), municípios do estado de São Paulo",
       subtitle = "Período de 2010 a 2022",
       caption = "Fonte: IEPS, PNI, TabNet/DATASUS",
...
cor_vac_txmortes %>% 
  ggsave(file = here("Figuras", "Municípios_2010_2020_BCG_vs_Taxa_Mortes_Evitaveis.png"), 
         width = 20, height = 10)
```
