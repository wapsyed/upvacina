...
sipni_cobertura_municipios_1994_2023_2 = sipni_cobertura_municipios_1994_2023 %>% 
 mutate(codigo = as.numeric(str_extract(municipio, "\\d+")),  # Extrai apenas os números e converte para numérico
        nome = str_remove(municipio, "\\d+ "),            # Remove os números seguidos de um espaço
        mun_uf = "Município"
  ) %>% 
  select(!"...1":municipio)
